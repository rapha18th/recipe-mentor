...
stated recommendation. Then it trains, quantizes,
...
2. **Deterministic recall.** `recall.py` filters facts and observations by
   ...
   own step numbers.
3. **A real autonomous agent, with its own tools.** `agent_runner.py` gives a
...
4. **A decision the agent genuinely waits on, mid-run.**
   ...
   answer arrives, before the agent configures anything. Two real
   ...
   genuinely waiting." A mental model of the loop treats those as the same
   moment. The actual loop treats them as two events, one step apart.
5. **A reflective chat, no tools at all.** `agent/chat.py` answers "what's
   ...
   recorded history, weak results included.
...
stronger result would actually require: more real data.
...
projects recorded. Your answer changes what actually runs. One more
question, for feedback, comes after it finishes. The feedback is recorded
the same way every lesson is, so the next project of the same kind sees
it, unprompted, before it starts.
...
Vertex access is the Cloud Run service's own. Nothing GCP-related is
ever sent by the browser. Kaggle access is bring-your-own: each visitor's
username and API key are used once, for that one run, held only in memory,
and cleared the moment it ends. Kaggle's own terms don't allow sharing
one person's credentials to download data on someone else's behalf, so
this couldn't be a shared service credential the way Vertex is.
...
A dataset that matches neither shape gets a plain error back.
...
### Where the resulting artifacts land

Every run writes two kinds of output. The passport is durable: accuracy,
ONNX byte sizes, the quantization verdict, all recorded as facts. It lives
in `passports/lab_demo.json` locally, and in Firestore too whenever
`--store firestore` or the hosted interface writes it.

The model files are local disk artifacts: `model_fp32.onnx`,
`model_int8.onnx`, `report.json`. The autonomous path writes them under
`recipe_mentor/data/agent_runs/_run/<owner>__<slug>/`, keyed by the Kaggle
dataset. The two hand-built demo pipelines write theirs under
`recipe_mentor/data/maize_run/` and `recipe_mentor/data/mimii_run/`.

Running this locally, those files sit on your own disk for as long as you
keep them. The hosted interface runs on Cloud Run, and Cloud Run's
filesystem is ephemeral. This repo never uploads those files anywhere
durable on its own, so a hosted run's ONNX files exist only for the life
of that container instance. The passport survives in Firestore; the model
files don't survive a redeploy or a scaled-down instance.

That's why the run page has its own export: once a run finishes, a
"Download model files (.zip)" link appears, right on the page. It hits
`GET /api/run/{run_id}/artifacts`, zips up whatever that run actually
wrote (`model_fp32.onnx`, `model_int8.onnx`, `report.json`), and streams
it down. Use it before the container recycles; nothing else keeps a copy
for you.
...
- `docs/ADR_Recipe_Mentor_Vertex_Backend_2026-08-29.md`: the passport
  ...
  `recall.py` filters by a fixed tag; it never runs a retrieval query. The original Vertex
  routing decision.
...
- `docs/ADR_Recipe_Mentor_Autonomous_Agent_2026-08-30.md`: the autonomous
  agent path above. Why the tools are coarse, each one doing real work
  inside a single call. Why the tools enforce the recipe's discipline
  instead of trusting the model to. How a dynamically created project
  gets a `ProjectCard` without a second LLM call. Why the bookend
  clarifying question and feedback capture are asked by the harness. A
  same-day addendum covers how `ask_user` genuinely pauses the model
  mid-loop for a real decision, the two real concurrency bugs it took to
  get there, and the reflective chat feature built alongside it. Real
  numbers from verifying all of it live.

## What's real, and what's a known gap

Everything above has actually run. See the ADRs for the
